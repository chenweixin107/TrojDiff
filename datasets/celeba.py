...
class CelebA(VisionDataset):
    ...
    def __getitem__(self, index):
        ...
        for t in self.target_type:
            if t == "attr":
                # target.append(self.attr[index, :]) # original
                tmp_target = self.attr[index, 18] + 2*self.attr[index, 21] + 4*self.attr[index, 31]
                target.append(int(tmp_target)) # new, 8 classes
            elif t == "identity":
