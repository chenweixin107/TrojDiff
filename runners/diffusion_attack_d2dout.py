...
def saveImg(img, save_dir, Gray=True):
    imgPath = save_dir

    # original
    # 改写：torchvision.utils.save_image
    grid = tvu.make_grid(img, nrow=8, padding=2, pad_value=0, normalize=False, range=None, scale_each=False)
    ndarr = grid.mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy() # (32,32,3)
    im = Image.fromarray(ndarr)
    # im = T.Resize((28,28))(im)
    # im.show()
    if Gray:
...
class Diffusion(object):
    ...
    def train(self):
        ...
        if args.dataset == 'cifar10' or args.dataset == 'celeba':
            targetset, _ = get_targetset("MNIST", args, config)
        elif args.dataset == 'bedroom':
            targetset, _ = get_targetset("church", args, config)
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                # add target data
                x_tar = next(target_loader)
                x_tar = x_tar[0] # (12,3,32,32)
    ...
    def sample_fid_bd(self, model):
        ...
        with torch.no_grad():
            for _ in tqdm.tqdm(
                ...
                # save (mnist)
                for i in range(n):
                    saveImg(x[i], os.path.join(image_folder, f"{img_id}.png"))
                    img_id += 1
    ...
    def sample_sequence_bd(self, model):
        ...
        for i in range(x.shape[1]):
            tvu.save_image(x[1:,i], os.path.join('/home/username/ddim/images/d2dout', f"img{i}_process.png"), nrow=50)#[100,3,32,32]
            # for j in range(x.shape[0]):
            #     tvu.save_image(
            #         x[j,i], os.path.join('/home/username/ddim/images/d2dout', f"img{i}_timestep{j}.png")#[3,32,32]
            #     )
